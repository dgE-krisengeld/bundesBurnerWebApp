...
export function LoadWalletComponent({
  jsonWallet,
  onDecrypt,
  decrypting,
  error,
  ...
  return (
    <div style={{ margin: "20px 0", justifyContent: "flex-start" }}>
      {error && (
        <div style={{ padding: 20, background: "#ee1a28", fontWeight: "bold" }}>
          {error}
        </div>
      )}
      {decrypting && (
        ...
        >
          <div style={{ marginTop: 200, textAlign: "center", color: "#FFF" }}>
            Bitte warten, wir entsperren Ihren Wallet.
          </div>
        </div>
      )}
